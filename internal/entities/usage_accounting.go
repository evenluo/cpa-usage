...
// canonical facts remain separate from the existing Cost/legacy token columns.
type UsageAccounting struct {
	AccountingVersion           *int64
	TokenBreakdownPresent       bool   `gorm:"not null;default:false"`
	AccountingMalformed         bool   `gorm:"not null;default:false"`
