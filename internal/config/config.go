...
type Config struct {
	...
	// AuthSessionSecret 是跨服务校验登录 session 的 HMAC 签名密钥；空值表示使用本进程内存 session。
	AuthSessionSecret string
	// AuthSessionCookieName 是登录 session cookie 名称。
	AuthSessionCookieName string
	// AuthSessionCookieDomain 是登录 session cookie domain；空值表示当前 host-only cookie。
	AuthSessionCookieDomain string
	// AuthSessionCookiePath 是登录 session cookie path；空值时按 AppBasePath 派生。
...
func Load(options LoadOptions) (*Config, error) {
	...
	authSessionCookiePath, err := normalizeCookiePath(strings.TrimSpace(os.Getenv("AUTH_SESSION_COOKIE_PATH")))
	if err != nil {
		return nil, fmt.Errorf("AUTH_SESSION_COOKIE_PATH is invalid: %w", err)
	}
...
func normalizeCookiePath(value string) (string, error) {
	if value == "" {
		return "", nil
	}
	if !strings.HasPrefix(value, "/") {
