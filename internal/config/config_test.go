...
func TestLoadFromEnvAppliesDefaults(t *testing.T) {
	...
	if cfg.AuthSessionSecret != "" {
		t.Fatalf("expected default auth session secret to be empty, got %q", cfg.AuthSessionSecret)
	}
	if cfg.AuthSessionCookieName != "cpa_usage_session" {
		t.Fatalf("expected default auth session cookie name cpa_usage_session, got %q", cfg.AuthSessionCookieName)
	}
	if cfg.AuthSessionCookieDomain != "" || cfg.AuthSessionCookiePath != "" {
		t.Fatalf("expected default auth session cookie scope to be empty, got domain=%q path=%q", cfg.AuthSessionCookieDomain, cfg.AuthSessionCookiePath)
	}
...
func TestLoadFromEnvRequiresCriticalValues(t *testing.T) {
	...
	t.Run("short auth session secret when auth enabled", func(t *testing.T) {
		t.Setenv("CPA_BASE_URL", "http://127.0.0.1:"+cpa.ManagementRedisDefaultPort)
		t.Setenv("CPA_MANAGEMENT_KEY", "secret")
		t.Setenv("AUTH_ENABLED", "true")
		...
		_, err := LoadFromEnv()
		if err == nil || err.Error() != "AUTH_SESSION_SECRET must be at least 32 characters when set" {
			t.Fatalf("expected AUTH_SESSION_SECRET length error, got %v", err)
		}
	})

	t.Run("invalid auth session cookie path", func(t *testing.T) {
		t.Setenv("CPA_BASE_URL", "http://127.0.0.1:"+cpa.ManagementRedisDefaultPort)
		t.Setenv("CPA_MANAGEMENT_KEY", "secret")
		t.Setenv("AUTH_SESSION_COOKIE_PATH", "usage")

		_, err := LoadFromEnv()
		if err == nil || err.Error() != "AUTH_SESSION_COOKIE_PATH is invalid: must start with '/'" {
			t.Fatalf("expected AUTH_SESSION_COOKIE_PATH validation error, got %v", err)
		}
	})
}
...
func TestLoadFromEnvParsesOverrides(t *testing.T) {
	...
	t.Setenv("AUTH_SESSION_COOKIE_PATH", "/")
