...
`v7.2.152-passive-quota.json` covers the supported `quota` / `model_quotas` response shape and observed provider units. `without-passive-quota.json` covers a supported management response with no observed passive quota signal. They contain no production identity, credential, request, provider call or database data.
