...
`v7.2.152-passive-quota.json` covers the supported `quota` / `model_quotas` response shape and observed provider units. `v7.2.62-no-passive-quota.json` records the absence of this shape in the pinned v7.2.62 baseline. They contain no production identity, credential, request, provider call or database data.
