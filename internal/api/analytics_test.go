	...
	return &repodto.AnalyticsSummarySnapshot{
		Summary: repodto.AnalyticsSummary{
			...
			Accounting: repodto.AnalyticsAccountingSummary{
				TotalAttempts: 3, ValidAttempts: 3, CoveragePct: &coverage,
				States:       repodto.AnalyticsAccountingStates{Valid: 3},
				ValidQuality: repodto.AnalyticsAccountingValidQuality{Complete: 3},
				Composition: repodto.AnalyticsAccountingComposition{
					TotalTokens: 2_100_100,
					Input:       repodto.AnalyticsAccountingInputComposition{TotalTokens: 1_500_100, UncachedTokens: 1_400_100, CacheReadTokens: 100_000},
					Output:      repodto.AnalyticsAccountingOutputComposition{TotalTokens: 600_000, NonReasoningTokens: 500_000, ReasoningTokens: 100_000},
				},
			},
		},
		Trend: []repodto.AnalyticsTrendPoint{{
			...
			ReasoningTokens: 100_000,
			CachedTokens:    100_000,
			RequestCount:    1, CanonicalValidAttempts: 1,
			SuccessCount:  1,
			FailureCount:  0,
			CostAvailable: true,
		...
		KeyAliasBreakdown: []repodto.AnalyticsKeyAliasBreakdown{{
			...
			TotalCost:      2.45,
			TotalTokens:    2_100_100,
			RequestCount:   3, CanonicalValidAttempts: 3,
			SuccessCount:  2,
			FailureCount:  1,
			SuccessRate:   66.6666666667,
...
func TestAnalyticsSummaryRouteReturnsSummaryTrendAndRangeMetadata(t *testing.T) {
	...
	provider := &analyticsStub{snapshot: &repodto.AnalyticsSummarySnapshot{
		Summary: repodto.AnalyticsSummary{
			...
			CacheReadCoverage:   100,
			CacheReadTokens:     100_000,
			Accounting:          analyticsSummaryContractSnapshot(start).Summary.Accounting,
		},
		Trend: []repodto.AnalyticsTrendPoint{{
			Label:           "2026-05-11",
			BucketStart:     start,
...
func TestAnalyticsSummaryRouteReturnsEmptyPayloadWithoutProvider(t *testing.T) {
	...
	body := rec.Body.String()
	if !contains(body, `"trend":[]`) || !contains(body, `"model_distribution":[]`) || !contains(body, `"time_breakdown":[]`) || !contains(body, `"provider_options":[]`) || !contains(body, `"cost_status":"unavailable"`) {
