...
func NewWithConfig(cfg config.Config) (*App, error) {
	...
	appInstance.Router = api.NewRouter(
		...
		authHandler,
		cfg.AppBasePath,
		api.OptionalProviders{Analytics: analyticsReader, UsageIdentity: usageIdentityReader, KeyAlias: keyAliasService, Quota: quotaService, AccountStatus: accountStatusService, ModelSupport: modelSupportService, RollupBackfill: rollupBackfillReader, Metrics: appInstance},
	)
