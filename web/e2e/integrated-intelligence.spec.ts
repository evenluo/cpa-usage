...
test("fixed 24-hour diagnostics open the matching request evidence selection", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText(/excluded: Missing 7/)).toBeVisible()
...
test("dark responsive Live Capacity separates stored evidence and loads model support explicitly", async ({ page }, testInfo) => {
  ...
  // One merged 5h meter: the newer reported reading replaces the older manual observation.
  ...
  // The active limit, subscription end, and timing lines live
  ...
  await expect(capacity.getByRole("region", { name: "Model request observations", exact: true })).toHaveCount(0)
  await expect(capacity.locator("time[datetime='2026-09-07T07:30:00Z']")).toHaveCount(0)
  ...
  expect(requests.filter((request) => request.path === "/quota/observations")).toHaveLength(1)
