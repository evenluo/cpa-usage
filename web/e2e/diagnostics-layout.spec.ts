...
for (const theme of ["light", "dark"]) {
  test(`${theme} diagnostics prioritize visible comparisons and compact investigation`, async ({ page }, testInfo) => {
    ...
    await expect(performanceCard.getByText("994 of 994")).toBeVisible()
    await sampleToggle.click()
    ...
    await expect(detail).toContainText("0 of 0")
    expect(await metricControls.evaluate((element) => element.getBoundingClientRect().top + window.scrollY)).toBeCloseTo(controlsTop, 1)
    await page.keyboard.press("Escape")
    ...
    expect(await metricControls.evaluate((element) => element.getBoundingClientRect().top + window.scrollY)).toBeCloseTo(controlsTop, 1)
    await metricControls.getByRole("button", { name: "Latency", exact: true }).click()
    ...
    const mappingDetails = page.locator("details").filter({ has: page.locator("summary").filter({ hasText: "Model mappings" }) })
    ...
    await expect(mappingDetails.getByText(/Same name does not mean it was routed that way/)).not.toBeVisible()
    await mappingDetails.getByText("About these mappings", { exact: true }).click()
    await expect(mappingDetails.getByText(/Same name does not mean it was routed that way/)).toBeVisible()
    await mappingDetails.getByText("About these mappings", { exact: true }).click()
...
test("performance summary stays stable at intermediate widths with numeric TPS", async ({ page }) => {
  ...
  for (const width of [360, 430, 540, 640, 820, 1024, 1440]) {
    ...
    await controls.getByRole("button", { name: "Latency", exact: true }).click()
