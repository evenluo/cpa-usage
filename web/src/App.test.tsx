...
describe('App', () => {
  ...
  it('renders analytics KPI and trend data from the analytics API', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('$0.49').length).toBeGreaterThanOrEqual(1))
    ...
    const trendSummary = screen.getByLabelText('Trend summary strip')
    expect(within(trendSummary).getByText('Avg Cost / Hour')).toBeInTheDocument()
    expect(within(trendSummary).getByText('Avg tokens / Hour')).toBeInTheDocument()
    expect(within(trendSummary).getByText('Peak Cost / Hour')).toBeInTheDocument()
    ...
    expect(within(trendSummary).getByText('2.1M')).toBeInTheDocument()
    expect(screen.queryByLabelText('Trend point detail')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Show trend details/ })).not.toBeInTheDocument()
    const trendBucketDetails = screen.getByLabelText('Trend bucket keyboard details')
    fireEvent.click(within(trendBucketDetails).getByText('Bucket details'))
    expect(within(trendBucketDetails).getByText('05-12')).toBeInTheDocument()
    ...
    expect(within(screen.getByLabelText('Trend summary strip')).getByText('Avg Cost / Day')).toBeInTheDocument()
  })
