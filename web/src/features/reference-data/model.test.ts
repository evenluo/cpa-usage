...
describe("Reference Data feature model", () => {
  ...
  it("normalizes shared Reference Data HTTP contract fixtures", () => {
    const apiKeyFromContract = apiKeyAliasTargetsFixture.api_keys[0] as APIKeyAliasTarget
    const accountFromContract = usageIdentitiesFixture.identities[0] as KeyIdentity

    expect(normalizeAPIKeyRows([apiKeyFromContract])[0]).toMatchObject({
      id: "redacted_api_bca22c540d80",
      alias: "Agent API Key",
    ...
    expect(normalizeAccountKeyRows([accountFromContract])[0]).toMatchObject({
      id: "42",
  ...
  it("builds Key Alias scope, draft, save, and clear commands", () => {
    const apiRows = normalizeAPIKeyRows([apiKey])
    const accountRows = normalizeAccountKeyRows([accountKey])
    ...
    expect(buildKeyAliasSaveCommand("account", accountRows[0], "Account Ops")).toEqual({
      valid: true,
      scope: "account",
      id: 7,
      alias: "Account Ops",
    })
    expect(buildKeyAliasSaveCommand("api-key", apiRows[0], "  ")).toEqual({ valid: false, reason: "empty" })
    expect(buildKeyAliasClearCommand("account", accountRows[0])).toEqual({
      valid: true,
      scope: "account",
      id: 7,
      alias: "",
  ...
  it("prioritizes missing Cost Rates and creates drafts from configured rates", () => {
    ...
    expect(nextCostRateDrafts("configured-model", pricingMap, {}, "prompt", "9")).toEqual({
      "configured-model": { prompt: "9", completion: "2", cache: "0.5" },
    })
  })
  ...
  it("requires complete non-negative Cost Rate drafts before building save payloads", () => {
    ...
    expect(buildCostRateSaveCommand("configured-model", { prompt: "", completion: "1", cache: "0" })).toEqual({ valid: false, reason: "missing" })
    expect(buildCostRateSaveCommand("configured-model", { prompt: "1", completion: "2.5", cache: "0" })).toEqual({
      valid: true,
      payload: {
        model: "configured-model",
        prompt_price_per_1m: 1,
