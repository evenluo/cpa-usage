      ...
      <CardContent className="min-h-0 min-w-0 flex-1 p-4 pt-0">
        {!hasCompleteData && isLoading ? (
          ...
          <div className="flex h-full min-w-0 flex-col justify-between gap-3">
            ...
            <Link
              to="/requests"
              search={{ provider, model: "", modelAlias: "", account: "", endpoint: "", status: "", requestId: "", windowEnd: data?.window_end ?? "", result: "" }}
              className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg text-xs font-medium text-terracotta-700 transition-colors hover:bg-terracotta-500/10 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-terracotta-500 dark:text-terracotta-300"
            >
              View all attempts
