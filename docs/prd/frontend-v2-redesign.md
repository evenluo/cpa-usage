...
The current CPA Usage frontend (`web/`) is a single 1,800-line `App.tsx` file that lacks visual sophistication and polished interaction design. Users frequently interact with the dashboard to monitor AI API consumption, but the current interface feels utilitarian rather than delightful. Key pain points include:
...
Rebuild the frontend in a new `web-v2/` directory with a **Claude-inspired editorial aesthetic** — warm terracotta accents, clean typography, generous whitespace, and a reading-flow information architecture. The new frontend will:
...
- **Test tooling**: Vitest (already configured in the existing `web/` package; adopt same for `web-v2/`).
- **Prior art**: The existing `web/` directory uses Vitest + React Testing Library. Follow the same patterns.
...
8. **Migration of existing `web/` directory** — `web-v2/` is a fresh build. The old `web/` remains untouched during development.
...
- The build output (`web-v2/dist/`) will eventually replace `web/dist/` in the Go static file serving pipeline. The Go backend's `router.go` serves `index.html` with base path injection — this mechanism remains unchanged.
