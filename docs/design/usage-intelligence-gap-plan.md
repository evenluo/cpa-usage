...
| Frontend types/state | `web/src/App.tsx` | Add `TimeGranularity` state; request `granularity`; map comparison and heatmap payloads. |
| Frontend charts | `web/src/components/charts/index.tsx` | Upgrade the primary Cost/Tokens chart, add KPI sparklines if needed, and add a heatmap component. |
| Frontend tests | `web/src/App.test.tsx`, `web/src/components/charts/index.test.tsx` | Cover default Hour query, Day switching, comparison rendering, and date-by-hour heatmap rendering. |
...
- Run frontend typecheck and build: `npm --prefix ./web run typecheck` and `npm --prefix ./web run build`.
...
- Before rebuild, record the current remote baseline:
  ...
  - current compose checksum from `/etc/dokploy/compose/cpa-cliproxyapi-hazmcp/code/docker-compose.yml`
  - current public smoke results for `/cpa-usage/healthz`, `/cpa-usage/`, and `/usage/healthz`
- Rebuild the `cpa-usage` service on maxtap through the existing Dokploy compose setup.
- Do not change `/usage`.
...
```text
https://cpa.maxtap.net/cpa-usage/healthz
https://cpa.maxtap.net/cpa-usage/api/v1/analytics/summary?range=7d&granularity=hour
https://cpa.maxtap.net/cpa-usage/api/v1/analytics/summary?range=7d&granularity=day
https://cpa.maxtap.net/cpa-usage/
https://cpa.maxtap.net/usage/healthz
```
