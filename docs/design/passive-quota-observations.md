...
The UI displays explicitly named `gpt-reserve` account limits as **Luna Reserve** in the card's main area. The raw identifier remains visible, and the original label remains the key for observation matching. The explanation is **Extra Luna usage after regular usage is exhausted.** If no account reading has been collected, the card displays **Luna Reserve (gpt-reserve)** and **No reading**, without a meter, inferred window or reset time. Its explanation is **No Reserve reading has been collected. Availability is unknown.** Absence in the stored observation does not establish whether OpenAI currently reports Reserve; an explicit refresh may collect a reading previously discarded by an older version.
