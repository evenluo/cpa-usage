...
- The [accounting-v2 extension](../design/usage-accounting-contract.md) preserves typed canonical/execution facts. Unsupported or malformed v2 messages use the existing explicit decode_failed isolation. There is no legacy-success fallback or malformed-field sentinel protocol; the destructive-pop boundary is unchanged.
