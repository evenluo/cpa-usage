...
- When the local runner provides Redis inbox processing metrics, throughput is exposed as cumulative counters (`redis_events_processed_total`, `redis_events_processed_batches_total`, `redis_events_last_processed_at`) plus a derived `redis_events_processing_rate_per_minute` computed from the delta between adjacent snapshot requests. Failed batches and empty batches (no rows to process) are not counted, so retries and idle polling do not inflate throughput or keep `redis_events_last_processed_at` advancing while idle. Without that local provider, these fields are omitted rather than represented as zero. A first comparable scrape has no rate; a later measured zero rate is emitted as `0`.
