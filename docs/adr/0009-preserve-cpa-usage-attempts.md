...
The [Accounting v2 direct cut](../design/accounting-v2-direct-cut.md) supersedes the historical `queue` event-key adapter. Its one-time migration requires the deployed consumer to finish every local pending/process_failed inbox row before upgrade. The new binary always assigns inbox-row attempt identity; it does not support unpublished intermediate databases or infer an identity mode at runtime. Existing historical event keys remain untouched.
