...
- Supported CPA accounting v2 preserves canonical non-overlapping token buckets, upstream quality, both schema versions, and optional generate/stream and separate requested/response tier facts. Canonical validity and quality are distinct; historical, absent, malformed, unsupported, or inconsistent evidence never becomes fabricated complete accounting. The repository owns the [per-attempt interpretation contract](docs/design/usage-accounting-contract.md); existing token scalar and Cost meanings remain unchanged.
