...
- **Attempt performance** breakdown plots select successful latency, failed latency, generating-streaming TTFT, or Output TPS locally. Each dimension uses its own labelled linear axis for p50/p95, keeps all returned groups ordered by attempt volume, and shows sample counts and coverage. These are frequent groups, not a slowest ranking; absent percentiles have no plotted point. Unscoped Output TPS remains per-provider numeric evidence without a shared comparison plot.
